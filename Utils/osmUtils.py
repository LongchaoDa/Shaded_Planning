...
'''
    Reads the link.csv file, return a dictionary with keys linkIDs,
    and value as object of linkID, fromNode, toNode, and lineString(geometry) of that path, in array form.
'''
def getLineStringDict(filePath):
    links = readLinkCSVFile(filePath);
    lineStringDict = {};
    for link in links:
        lineStringDict[link[1]] = {
            "linkID": link[1],
            "fromNode": link[3],
            "toNode": link[4],
            "lineString": (link[12])
        }
    return lineStringDict
