...
def downloadSegmentedMap(latTop, latBottom, longLeft, longRight, zoom, imgSize, dirName, areaName):
    ...
    # For the first image, find lat,long of the edges, to estimate center for the next image.
    ...
    # Looping over all center coordinates, and retrieving images.
    currLat = latTop
    xIndex = 1

    # Image Directory that stores imageName: {lat, long} for each image(centre coordinates)
    # Also stores the radius or length of the side of each image in meters.
    ...
    # Storing the number of rows and cols in the image array
